...
st.sidebar.subheader("Configuración de Pluma Telescópica")
# Parámetros por defecto para mantener consistencia con las secciones estructurales
p_ext_val = st.sidebar.text_input("Porcentajes de Extensión (separados por coma)", "0.0, 46.0, 46.0, 46.0, 46.0, 46.0")
l_max_val = st.sidebar.text_input("Longitudes Máx. Secciones (m)", "6.0, 5.0, 5.0, 5.0, 5.0, 5.0")
m_base_val = st.sidebar.text_input("Masas Base Secciones (kg)", "1200.0, 800.0, 700.0, 600.0, 500.0, 400.0")
...
if st.button("Ejecutar Análisis y Visualización"):
    ...
    try:
        p_ext = [float(x.strip()) for x in p_ext_val.split(',')]
        l_max = [float(x.strip()) for x in l_max_val.split(',')]
        m_base = [float(x.strip()) for x in m_base_val.split(',')]
    except ValueError:
        st.error("Error en los formatos de las listas de la pluma. Revisa que sean números separados por comas.")
        st.stop()
    ...
    pluma_op = PlumaTelescopica(porcentajes_extension=p_ext, longitudes_max_secciones=l_max, masas_base_secciones=m_base)
    ...
    st.write(f"**Longitud Total Pluma Calculada:** {pluma_op.longitud_total:.2f} m | **Masa Pluma:** {pluma_op.masa_total_pluma:.2f} kg")
    ...
    ax.set_title(f"Giro: {grua_op.giro_torreta_deg:.1f}° - L_pluma: {pluma_op.longitud_total:.2f}m", fontsize=10, fontweight='bold')
